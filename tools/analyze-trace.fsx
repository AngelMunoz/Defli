...
let private shorten (n: string) =
  if n.Length <= 110 then n
  else n.Substring(0, 107) + "..."
...
let path =
  ...
  | [| _; p |] -> p
  | _ -> failwith "usage: dotnet fsi tools/analyze-trace.fsx <trace.speedscope.json>"
...
for profile in profiles do
  ...
  let events =
    ...
    |> Seq.map (fun e ->
      struct (e.GetProperty("type").GetString(), e.GetProperty("frame").GetInt32(), e.GetProperty("at").GetDouble()))
    |> Seq.toArray
  ...
  if events.Length = 0 then
    ...
    let isAllocLeaf (n: string) = n.Contains "ArrayModule.ZeroCreate" || n.Contains "ArrayModule.Create"
    let isStringLeaf (n: string) = n.Contains "String.Concat" || n.Contains "StringPrintfEnv"
    ...
    for struct (t, f, at) in events do
      ...
      | Some top ->
        let span = at - prevAt
        exclusive[top] <- (if exclusive.ContainsKey top then exclusive[top] else 0.0) + span
      | None -> ()
      ...
      if t = "O" then
        ...
        if isAllocLeaf (frameName f) then
          ...
          | Some cf ->
            ...
            allocCallers[key] <- (if allocCallers.ContainsKey key then allocCallers[key] else 0) + 1
          | None -> ()
        elif isStringLeaf (frameName f) then
          match caller with
          | Some cf ->
            let key = shorten (frameName cf)
            stringCallers[key] <- (if stringCallers.ContainsKey key then stringCallers[key] else 0) + 1
          | None -> ()
      else // "C"
        if stack.Count > 0 then
          ...
          let span = at - openAt
          inclusive[cf] <- (if inclusive.ContainsKey cf then inclusive[cf] else 0.0) + span

      prevAt <- at

    printfn ""
    printfn "════ profile: %s ════  duration %.1f ms (%.2f s)" name duration (duration / 1000.0)
    ...
    printfn "  %6.1f%%  (idle / native wait / unaccounted)" (100.0 * idle / duration)

    let adaptiveSlopSpan =
      exclusive
      |> Seq.filter (fun kv -> (moduleOf (frameName kv.Key)) = "AdaptiveSlop.Core")
      |> Seq.sumBy (fun kv -> kv.Value)

    printfn ""
    printfn "  → AdaptiveSlop total: %.1f%% of wall time (%.2f s of %.2f s)"
    ...
    printfn "  samples total        : %d (%.1f s of busy time at 1 kHz)"
      sampleCount
      (float sampleCount / 1000.0)
    printfn "  samples in AdaptiveSlop: %d (%.1f%% of busy time)"
      adaptiveSlopSamples
      (100.0 * float adaptiveSlopSamples / float sampleCount)
    printfn "  samples idle/native   : %d" idleSamples
    printfn ""
    printfn "  busy share of wall: %.1f%%  →  AdaptiveSlop wall share ≈ %.1f%%"
    ...
    |> Seq.truncate 14
    |> Seq.iter (fun kv ->
      printfn "  %6.1f%%  %s" (100.0 * kv.Value / duration) (shorten (frameName kv.Key)))
