...
let tests =
  testList "Map" [
    ...
    testCase "cellAt picks the CONTAINING cell (no half-tile shift)" (fun () ->
      ...
      // bottom halves.
      match Application.cellAt (System.Numerics.Vector2(100f, 84f)) terrain with
      | ValueSome struct (1, 1) -> ()
      | other -> failtestf "expected (1,1), got %A" other

      match Application.cellAt (System.Numerics.Vector2(127f, 127f)) terrain with
      | ValueSome struct (1, 1) -> ()
      | other -> failtestf "expected (1,1), got %A" other
      ...
      Expect.isTrue
        (Application.cellAt
          (System.Numerics.Vector2(2000f, 2000f))
          terrain).IsNone
        "past the far edge")
    ...
    testCase "decorations: props + dirt blends exist and stay off the road" (fun () ->
...
let proceduralTests =
  let procCfg = { cfg with MapVariant = MapVariant.Procedural }
  ...
  testList "Map (procedural)" [
    ...
    testCase "floodFill from spawn reaches the base" (fun () ->
      ...
      let reachable =
        Grid2DSpatial.floodFill sx sy isPassable deco
    ...
    testCase "obstacles block buildability; road cells never hold one" (fun () ->
      let deco = MapModel.decorations pmap
...
let probeTests =
  ...
  testList "Map (probe)" [
    testCase "dump path vs obstacles" (fun () ->
      ...
      CellGrid2D.iter
        (fun x y t ->
          if t.Decoration.IsSome && not t.Buildable then
            obs <- obs + 1
            let onPath = pathGrid |> CellGrid2D.get x y |> ValueOption.exists _.IsPath
            if onPath then
              onRoad <- onRoad + 1
      ...
      CellGrid2D.iter
        (fun x y t -> if t.IsPath then path <- path + 1)
        pathGrid
    ...
    testCase "game seed 42 road shape" (fun () ->
      ...
      printfn "PROBE42 spawn=%A base=%A len=%d" g.SpawnCell g.BaseCell g.Path.Length
