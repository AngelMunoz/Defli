...
let private spawnAt (m: ProjectilesModel) (pos: Vector2) =
  let struct (m', _) =
    Projectiles.update
      (ProjectileMsg.Spawn(pos, target, 5, 100f))
      m
...
let tests =
  testList "Projectiles" [
    ...
    testCase "homing: seeks the target's live position and impacts" (fun () ->
      ...
      let struct (m2, _) = Projectiles.tick 0.1f m (positionsAt (Vector2(50f, 0f)))
    ...
    testCase "target despawned mid-flight → row removed, no impact" (fun () ->
      ...
      let struct (m2, events) = Projectiles.tick 0.1f m (Dictionary<int<EnemyId>, Vector2>())
    ...
    testCase "Homing projection: render row tracks the target's live position" (fun () ->
      ...
      let projections =
        Projections(enemies', towers, projectiles', economy, map.Grid, hover)
      ...
      let struct (enemies2, _) =
        Enemies.Enemies.tick 1.0f enemies' map.Path

      let rows2 = projections.Homing |> AMap.getValue

      for KeyValueV(pid, v) in rows2 do
        Expect.equal v.TargetPos (map.Path[0].X + 90f |> fun x -> Vector2(x, map.Path[0].Y)) "tracked movement"
