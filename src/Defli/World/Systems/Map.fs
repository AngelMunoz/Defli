...
module MapModel =
  ...
  let private scatterObstacles
    ...
    : unit =
    Layout.scatterStamp count seed (fun s ->
      let gx = s.OffsetX
      let gy = s.OffsetY
  ...
  let private scatterVisualProps
    ...
    Layout.scatterStamp count seed (fun s ->
      let gx = s.OffsetX
      let gy = s.OffsetY
      let onPath = pathLayer |> CellGrid2D.get gx gy |> ValueOption.exists _.IsPath
          ...
          for struct (nx, ny) in Grid2DSpatial.neighbors4 x y pathLayer do
            ...
            if freeGrass && empty && hashRoll nx ny seed < 0.45 then
              ...
              let frame =
                ...
                elif dx < 0 then Tiles.dirtPatchOnGrassLeft
                elif dx > 0 then Tiles.dirtPatchOnGrassRight
                elif dy < 0 then Tiles.dirtPatchOnGrassTop
                else Tiles.dirtPatchOnGrassBottom
  ...
  let private tryProcedural
    (cfg: WorldConfig)
    (seed: int)
    : struct (LayeredGrid2D<MapTile> * struct (int * int)[] * struct (int * int) * struct (int * int)) voption =
    ...
    let struct (deco, _) = LayeredGrid2D.getOrAddLayer MapLayers.Decorations grid
    let struct (buildable, _) = LayeredGrid2D.getOrAddLayer MapLayers.Buildable grid
  ...
  let private buildModel
    ...
    : MapModel =
    let cellSize = Vector2(float32 Tiles.TileSize, float32 Tiles.TileSize)
    let struct (terrainLayer, _) = LayeredGrid2D.getOrAddLayer MapLayers.Terrain grid
    let struct (pathLayer, _) = LayeredGrid2D.getOrAddLayer MapLayers.Path grid
    let struct (deco, _) = LayeredGrid2D.getOrAddLayer MapLayers.Decorations grid
