...
module Map =
    ...
    let private pathFrame (grid: CellGrid2D<MapTile>) (x: int) (y: int) : struct (TileInfo * float32) =
        let isPath x y =
    ...
    let private grassVariant (x: int) (y: int) = Tiles.groundGrass[(x * 7 + y * 13) % 3]

    let view (ctx: GameContext) (model: MapModel) (hoverCell: struct (int * int) voption) (buffer: RenderBuffer2D) =
        ...
        buffer
            .sprite(
                SpriteState.create (tex, Rectangle(basePos.X, basePos.Y, size, size), Tiles.turretMountEmpty.Rect)
                |> SpriteState.withLayer Layers.Path
            )
            .drop ()
        ...
        | ValueSome(struct (hx, hy)) ->
            ...
            buffer.rectOutline(p.X, p.Y, size, size, Color.White, thickness = 2f, layer = Layers.Hud).drop ()
        | ValueNone -> ()
